...
HTML;
		...
		foreach($images as $image) {
			...
			$output .= '<img '.$thumb_img.' src="'.$image_src.'"/>';
			...
			if($image['type'] == 'slideshow'){
				...
				if($video_type == 'iframe' && !empty($iframe_src)){
					...
					$output .= '<div class="video_kenburn_wrap">';
					$output .= '<div class="video_video">';
					$output .= '<iframe class="video_clip" id="video" src="'.get_option('site_url').'?html5iframe=true&sliderid='.$post_id.'" allowfullscreen=""></iframe>';
					$output .= '</div>';
					$output .= '<div class="close"></div>';
					$output .= '</div>';
					$output .= '</div>';
			...
			if(! empty($image['desc'])){
				if( $image['type']=='slideshow' ){
					...
					$desc_bg = get_post_meta($post_id, '_ken_desc_bg', true);
					$desc_bg = empty($desc_bg) ? '' : 'background-color:'.$desc_bg.';';
					$desc_color = get_post_meta($post_id, '_ken_desc_color', true);
					...
					$output .= '<div class="ken-desc '.$desc_position .' '.$desc_transition.'" style="'.$desc_bg.$desc_color.'">';
