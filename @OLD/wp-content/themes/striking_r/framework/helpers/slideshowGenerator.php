...
class SlideshowGenerator {
	...
	public static function render($type, $category = '', $color = '',$number ='-2',$text = false) {
		...
		$output = '<div id="feature" class="with_shadow '.$stype_class.'" '.$color.'>';
	...
	public static function get_slideshow($stype,$images,$options){
		$slideshow = self::$instances[$stype];
		return $slideshow->render($images,$options);
	}
	...
	public static function get_options($stype,$tmpl){
		...
		foreach ($defined as $opt) {
			...
			$options[$slugs[2]] = theme_get_option('slideshow',$opt['id']);
		}
		return $options;
	}
