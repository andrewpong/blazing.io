...
function theme_update($from, $to) {
	...
	if ( !$wp_compat ) {
		...
		return new WP_Error( 'wordpress_not_compatible', sprintf( __('The update cannot be installed because %1$s %2$s requires Wordpress version %3$s or higher. You are running version %4$s.','theme_front'), THEME_NAME, $theme_version, $required_wp_version, $wp_version ) );
	}
...
function _copy_theme_file($from, $to){
	...
	if ( ! $wp_filesystem->copy( $from, $to, true, FS_CHMOD_FILE) )
		return new WP_Error('copy_failed', __('Could not copy file.','theme_front'), $from);
}
...
function _copy_theme_file_mkdir($dir){
	...
	if ( !$wp_filesystem->mkdir($dir, FS_CHMOD_DIR) )
		return new WP_Error('mkdir_failed', __('Could not create directory.','theme_front'), $dir);
}
