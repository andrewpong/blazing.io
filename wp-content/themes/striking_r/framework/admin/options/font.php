...
<p>Striking has unparalleled choice of font display options for a website with approx 840 different fonts plus 400+ icons available via 5 different typography technologies: <br />
<br />
...
<h3>USAGE</h3>
<p>In this <b>General Settings Tab</b> is the <b>Site Font Family</b> setting which allows one to choose from 9 common web safe fonts to act as the base font for the website. &nbsp;One does not need to do anything further after selecting the base site font -> choosing another font(s) for custom or descriptive text replacement is optional and depends on the degree of customization and individuality desired for website appearence.</p>
...
<p>Secondly, this premium theme is a rarity in that it allows more then one font technology to be active simultaneously for custom & descriptive replacement, but be advised depending on matters such as plugins in use, custom js & php employed in your site, and your host php version and settings it can lead to unpredictable website behaviour. &nbsp;Unless you are an advanced wordpress user with in-depth code and troubleshooting knowledge, we recommend not employing more then one custom font technology for replacement and <u>advise that any issues that arise from employing multiple font techonologies for font replacement simultaneously are not part of free product support for this theme.</u></p>
...
<p>As font awesome usage requires loading another css file, there is a setting for activing its usage -> the <b>Font Icons Integration</b> setting is found below. &nbsp;The reason for the setting is so that if one is not employing font awesome in the website, one is not hit with the css file loading burden to the site scripting (it is farily minimal but as they say in webdesign (and physics!) <em>&#34;Every nanosecond counts... :)&#34;</em>.</p>
<h3>DEFAULT ICON FONT SETS</h3>
<p>Striking also employs a variety of open source icon sets for use in the <b>Icon Text</b> and <b>Icon Link</b> shortcodes and for the icons used in the social media and contact widgets. &nbsp;One can also import other social media icon sets for use in the social media widget.</p>", 'theme_admin'),admin_url( 'admin.php?page=theme_color&tab=header')),
				"options" => array(
					array(
...
<p>Other then the requirement to load separate CSS files, the font awesome icon set code is fully integrated into the Striking core theme files and in fact the theme has one of the most complete integrations of the various font awesome abilities available anywhere.</p><p>NOTE that Striking also has several other open source font icon sets in use in some shortcodes and the social icons and contact widgets which are fully integrated into the theme core files and thus do not require separate activation; thus this setting has no involvement with those default icon sets -> they are already available the moment the theme was activated.</p>",'theme_admin'),
			...
			array(
				...
				"desc" => sprintf(__("<h3>INTRODUCTION</h3>
<p>Striking includes 29 settings below for adjusting different text types to a desired font size in the website. &nbsp;These settings are applied to that text type site wide.  &nbsp;However, if on any particular page one desires a font size setting to be different then the site setting, one can add some custom css in the custom css field in the <b>Page General Options Metabox/Page Design Settings Tab</b> found below the wp content editor in any page or post being edited. &nbsp;A benefit of this particular custom css field is that any css placed into it affects only that page or post being edited.</p>
<h3>CORRESPONDING TEXT COLORS</h3>
<p>In Striking, all the text types below have a corresponding theme color setting which can found in the <a href='%s' target='_blank'>Color Panel.</a> part of the array of over 110 theme color settings (all optional to adjust from their defaults) allowing for a unique font size and color palette for each website.",'theme_admin'),admin_url( 'admin.php?page=theme_color')),			
				"options" => array(
					array(
...
<p>The big upside of google fonts is the library has grown over time and Striking offers 738 google fonts (the entire library at time of publication of the theme) and google manages all the licensing of these fonts eliminating this concern for the user.</p>
<h3>USAGE</h3>
