...
HTML;
		...
		foreach(self::$slideTypes as $type => $label){
			...
			echo '<input type="text" id="custom-'.$type.'" class="add_custom" name="add_custom'.$type.'" pattern="([a-zA-Z\x7f-\xff][a-zA-Z0-9\x7f-\xff]*){0,1}" data-message="'.__('Please input a valid name which starts with a letter, followed by letters, numbers.','theme_front').'" maxlength="20" /><span class="validator-error"></span>';
