...
<p align='justify'>Finally, the portfolio items grouped into a portfolio category are entirely determined by choice ->a portfolio category may be used to group same type items or it can have a mixed group of portfolio types - there are no restrictions of any sort as to what portfolio items are placed into any portfolio category one creates.  &nbsp;Portfolio Categories are like shelves in a bookcase, what is put in them and how they are organized is all according to choice and convenience. &nbsp;Then one uses the Portfolio Shortcode to display the portfolio items, grouped by category(s), or selected individually, as part of the content.</p>", 'theme_admin'),admin_url( 'admin.php?page=theme_portfolio'),admin_url( 'admin.php?page=theme_portfolio'),admin_url( 'admin.php?page=theme_portfolio&tab=lightbox_dimension')),
				"options" => array(
					array(
...
<li><strong><u>LINK</u></strong> - The Link portfolio type is non-lightbox portfolio type where one wants to have the portfolio list thumbnail image link to something other then the single portfolio post webpage. So if one sets the Portfolio type to Link, then in the Link admin settings, one can choose linking directly to any site page, category, blog or custom post, or to link to any other url (internal or external). &nbsp;Like all other portfolio types, one sets a featured image so that a thumbnail appears in the list, which when clicked by a site visitor, takes them to the link set in the Link admin tab. </li>
<li><strong><u>PORTFOLIO GALLERY</u></strong> - this is where one can have gallery of grouped images, which will show up in the lightbox when the featured image in the list is clicked upon for enlargement. The Portfolio Gallery tab contains the functions for loading the gallery images.</li>
...
<p align='justify'>Sometimes there is confusion between the Link and Document portfolio types. &nbsp;The Link type is used to bring the site viewer to another webpage within a site or an external url when they click on thumbnail image, whereas the Document type is used to bring the viewer to the single portfolio post webpage.</p>",'theme_admin'),
					...
					array(
						...
						"desc" => __("<p align='justify'>Wordpress does not have a static page setting for a &#34;Portfolio&#34; page in the manner that it has for a home page and a blog page.  &nbsp;This setting will allow for accomplishing something similiar in that it allows for selection of a page to be the breadcrumbs parent page for all portfolio items that are created for the site.</p><p align='justify'>A common usage with this feature is to have created a &#34;My Portfolio / Portfolios / Products&#34; page and have the portfolio shortcode embedded, and thus the page will act as a static page for breadcrumbs, and a viewer visiting a portfolio post would see in the breadcrumbs &#34;&#34;My Portfolios -> Portfolio Post&#34;.</p><p align='justify'><em>NOTE</em> :  &nbsp;&nbsp;This setting is also found in the Portfolio Panel, but the setting in this metabox provides an override to a new breadcrumbs parent page just for this post item.",'theme_admin'),
					...
					array(
						...
						"desc" => sprintf(__("<p align='justify'>This will override portfolio default icon set in the <a href='%1s' target='_blank'>Portfolio Panel->General Tab - Effect setting</a>.</p>",'theme_admin'),admin_url('admin.php?page=theme_portfolio&tab=general')),
						"id" => "_icon",
						"default" => 'default',
						"options" => array(
					...
					array(
						...
						"desc" => __("<p align='justify'>There is already a link type portfolio, and a document portfolio type, and so what is the purpose of this setting? &nbsp;This is effectively a &#34;9th&#34; portfolio type as it allows you to use the Read More Button as an link alternative, but for any portfolio type!  &nbsp;If the Read More is enabled for the portfolio list and you choose from the linking options below, a site viewer clicking on the Read More button will follow to the link selected below, versus whatever is the action of clicking on the thumbnail (or Title if it is turned ON in the as it is also an active weblink to the single post webpage).</p><p align='justify'>This setting is most often used when one is inserting a portfolio list needing a different action for linking at the end of the post excerpt - for example the &#34;Read More Button Text&#34; has been changed (this ability is in the shortcode to create custom read more text) to say &#34;Buy Here&#34; and they are transported to the online store part of the website to purchase the item they have just viewed in the portfolio list.</p>",'theme_admin'),
						"id" => "_more_link",
						"default" => "",
			...
			array(
				...
				"options" => array(
					array(
						...
						"desc" => __("<p align='justify'>This setting allows one to substitute an alternate image for appearence in the lightbox in place of the featured image of the portfolio post. &nbsp;If not assigned, the featured image will appear in the lightbox.</p>",'theme_admin'),
						"id" => "_image",
						"button" => "Insert Image",
						"default" => '',
						"type" => "upload",
			...
			array(
				...
				"options" => array(
					array(
						...
						"desc" => __("<p align='justify'>Paste the full url of the YouTub, Vimeo or mp4 starting with http:// into the url field below. &nbsp;The mp4 ability is designed for self-hosted video only. &nbsp;If an mp4 video, it should be loaded up in advance by using the &#34;Add New Media&#34; function of the media library or by ftp into a subfolder in your site, and make a note of the file path for pasting into this url field.</p>",'theme_admin'),
						"size" => 30,
						"id" => "_video",
						"default" => '',
						"class" => 'full',
					...
					array(
						...
						"desc" => sprintf(__("<p align='justify'>If you specify a width below, this will override the portfolio video lightbox width default set in the <a href='%1s' target='_blank'>Portfolio Panel -> Lightbox Dimension Tab - Video Type Width</a> setting.</p> ",'theme_admin'),admin_url('admin.php?page=theme_portfolio&tab=lightbox_dimension')),
						"id" => "_video_width",
						"default" => "",
						"min" => 0,
						"max" => 960,
					...
					array(
						...
						"id" => "_video_autoplay",
						"desc" => sprintf(__("<p align='justify'>If you specify an autoplay function below, this will override the portfolio video autoplay default set in the <a href='%1s' target='_blank'>Portfolio Panel -> Video & Audio Tab - Video Autoplay</a> setting. &nbsp;It is important to remember that many browsers have a preloading requirement before a video will autoplay, and so if the video is a large one, the user may be left waiting for some (many!) seconds prior to autoplay commencing. &nbsp;Not all mobile browsers support autoplay.</p> ",'theme_admin'),admin_url('admin.php?page=theme_portfolio&tab=video_audio')),	
						"default" => 'default',
						"type" => "tritoggle"
					),
				),
			...
			array(
				...
				"options" => array(
					...
					"desc" => __("<p align='justify'>Paste the full url of the mp3 starting with http:// into the url field below. &nbsp;The audio file should be loaded up in advance by using the &#34;Add New Media&#34; function of the media library or by ftp into a subfolder in your site, and make a note of the file path for pasting into this url field.</p>",'theme_admin'),	
						"size" => 30,
						"id" => "_audio",
						"default" => '',
						"class" => 'full',
					...
					array(
						"name" => __("Autoplay",'theme_admin'),
						"id" => "_audio_autoplay",
						"desc" => sprintf(__("<p align='justify'>If you specify an autoplay function below, this will override the portfolio audio autoplay default set in the <a href='%1s' target='_blank'>Portfolio Panel -> Video & Audio Tab - Audio Autoplay</a> setting. &nbsp;It is important to remember that many browsers have a preloading requirement before an audio will autoplay, and so if the audio is a large one, the user may be left waiting for some (many!) seconds prior to autoplay commencing. &nbsp;Not all mobile browsers support autoplay.</p> ",'theme_admin'),admin_url('admin.php?page=theme_portfolio&tab=video_audio')),	
						"default" => 'default',
						"type" => "tritoggle"
					),
					array(
						...
						"id" => "_audio_loop",
						"desc" => sprintf(__("<p align='justify'>If you specify an audio looping function below, this will override the portfolio audio looping default set in the <a href='%1s' target='_blank'>Portfolio Panel -> Video & Audio Tab - Audio Loop</a> setting. &nbsp;Not all mobile browsers support looping.</p> ",'theme_admin'),admin_url('admin.php?page=theme_portfolio&tab=video_audio')),
						"default" => 'default',
						"type" => "tritoggle"
					),
				),
			...
			array(
				...
				"options" => array(
					...
					array(
						...
						"desc" => sprintf(__("<p align='justify'>If you specify a width below, this will override the portfolio lightbox width default set in the <a href='%1s' target='_blank'>Portfolio Panel -> Lightbox Dimension Tab - Lightbox Type Width</a> setting.</p> ",'theme_admin'),admin_url('admin.php?page=theme_portfolio&tab=lightbox_dimension')),	
						"id" => "_lightbox_width",
						"default" => "",
						"min" => 0,
						"max" => 960,
					...
					array(
						...
						"desc" => __("<p>For mobile platforms such as pads and phones, lightboxes are often not desirable and lightbox behaviour is very erratic from platform to platform due to extremely varied support by the mobile browsers for document viewing.  &nbsp;So you you have the option of providing a direct link to your lightbox content, and if the user is on a mobile device, it will be detected and instead of the content being shown in a lightbox, the content will be shown in the url window or they will be directed to the alternate link you have specified in the field below.</p>",'theme_admin'),
...
<p align='justify'> ** Office XML formats from 2007 and later sometimes can have a problem with the google viewer. &nbsp;Its a good idea to test xml types and where possible use the 2003 format instead.</p><p align='justify'>On rare ocassions, one might experience an error from Google indicating a bandwidth limit has been reached, etc. &nbsp;This is a google problem, and it goes away after a while as the google docs api refreshes its available resources periodically.</p>",'theme_admin'),
					...
					array(
						"name" => __("Lightbox Width",'theme_admin'),
						"desc" => sprintf(__("<p align='justify'>If you specify a width below, this will override the portfolio Doc type lightbox width default set in the <a href='%1s' target='_blank'>Portfolio Panel -> Lightbox Dimension Tab - Google Doc Type Width</a> setting.</p> ",'theme_admin'),admin_url('admin.php?page=theme_portfolio&tab=lightbox_dimension')),	
						"id" => "_gdoc_width",
						"default" => "",
						"min" => 0,
						"max" => 960,
			...
			array(
				...
				"options" => array(
					array(
						...
						"desc" => __("<p align='justify'>This setting allows the featured image to act as a direct link to another url, either internal to the website or external. &nbsp;Below select linking to a page, post, category, or link manually -> where one manually designates the url to be linked, usually an external url.  &nbsp;Upon making a selection another field will appear allowing one to choose the specific page, post or category in a dropdown list.  &nbsp;If manually is selected, a field will appear in which to enter the full url (including http://) for linking.</p>",'theme_admin'),
						"id" => "_link",
						"default" => "",
